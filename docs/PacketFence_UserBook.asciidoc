...
We will now add the possibility to filter corportate and BYOD devices. One way to do that is to use Machine Authentication (Windows devices only) and some VLAN/RADIUS filters for others `corporate' devices.

Machine Authentication via LDAP
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

To configure Machine Authentication, please configure a source LDAP with the setting *Username Attribube: servicePrincipalName* and make sure the *Base DN* is equal to where the `OU' where the computers you want to authenticate are.
...
Machine Authentication via filters
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Now that your Windows devices are able to be logged in with Machine Authentication you can class them as `corporate'. Let's say that you now need to add some `IPads' and `Linux computers' as `corporate' devices.
