...
We will now add the possibility to filter corportate and BYOD devices. One way to do that is to use Machine Authentication (Windows devices only) and for the other `corporate' devices use EAP-TLS and a specific LDAP attribute.
