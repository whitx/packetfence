...
We will now add the possibility to filter corportate and BYOD devices. One way to do that is to use Machine Authentication (Windows devices only) and for the other `corporate' devices use EAP-TLS and a specific LDAP attribute. To begin create a new PacketFence role, under `Users->Roles` in the tab configuration, call it `corporate`.
...
You can now create a new Portal Profile for EAP-TLS. Under the tab configuration, section `Main->Portal Profiles`, `Add profile` and select as a filter the Sub Connection Type as EAP-TLS, add your sources LDAP.
